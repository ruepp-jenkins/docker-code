...
- **Your home** — `/home/agent`, which on the host is `~/docker-code/opencode/`. Everything you keep
...
```bash
docker run --rm -v "$PWD":/src -w /src --user "$(id -u):$(id -g)" mcr.microsoft.com/dotnet/sdk:8.0 dotnet test
```
